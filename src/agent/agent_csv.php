...
$sql = "SELECT users.name,users.hurigana,users.sex,users.birthday,users.college,users.faculty,users.department,users.division,users.grad_year,users.prefecture,users.mail,users.phone FROM users INNER JOIN user_register_client AS r ON users.id = r.user_id WHERE r.client_id = :id && users.valid = 0 && DATE_FORMAT(users.created_at, '%Y%m') = DATE_FORMAT(NOW(), '%Y%m')  ORDER BY updated_at DESC";
$stmt = $pdo->prepare($sql);
$stmt->bindValue(":id", $_SESSION["id"]);
$stmt->execute();
