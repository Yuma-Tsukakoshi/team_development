  ...
  <form action="http://localhost:8080/user/user_info/user_thanks.php" method="post">
    <h1>お申し込み確認フォーム</h1>
        <div>
          ...
          <input type="text" name="name" id="name" class="form-control" value="<?=$_POST['name']?>" readonly/>
          <button class="edit-button" data-value="2">編集</button>
        </div>
        <label for="hurigana" class="form-label">ふりがな:</label>
          <input type="text" name="hurigana" id="hurigana" class="form-control" value="<?=$_POST['hurigana']?>" readonly/>
          <button class="edit-button" data-value="2">編集</button>
        </div>
        <div class="form-controll">
          <label for="email" class="form-label">メールアドレス:</label>
          <input type="email" name="email" id="email" class="form-control" value="<?=$_POST['email']?>" readonly/>
          <button class="edit-button" data-value="3">編集</button>
        </div>
        <div class="form-controll">
          <label for="phone" class="form-label">電話番号:</label>
          <input type="tel" name="phone" id="phone" class="form-control" pattern="\d{1,5}-\d{1,4}-\d{4,5}" title="電話番号は、市外局番からハイフン（-）を入れて記入してください。" value="<?=$_POST['phone']?>" readonly/>
          <button class="edit-button" data-value="4">編集</button>
        </div>
        <div class="form-controll">
          <fieldset class="form-control" readonly>
        ...
        <div class="form-controll">
          ...
          <input type="date" name="birthday" id="birthday" class="form-control" value="<?=$_POST['birthday']?>" readonly/>
        ...
        <div class="form-controll">
          ...
          <input type="text" name="college" id="college" class="form-control" value="<?=$_POST['college']?>" readonly/>
        ...
        <div class="form-controll">
          ...
          <input type="text" name="grad_year" id="grad_year" class="form-control" value="<?=$_POST['grad_year']?>" readonly/>
